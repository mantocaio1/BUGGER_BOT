...
exports.buildSuggestionsEmbed = buildSuggestionsEmbed;
exports.buildSuggestionsEmbeds = buildSuggestionsEmbeds;
const discord_js_1 = require("discord.js");
const suggestions_1 = require("../config/suggestions");
function buildSuggestionsEmbed(module) {
    if (module === "todos") {
        return buildFullGuideEmbed();
    }
    const info = suggestions_1.MODULE_SUGGESTIONS[module];
    const placeholders = [
        ...suggestions_1.PLACEHOLDERS.common,
        ...(suggestions_1.PLACEHOLDERS[module] ?? []),
    ];
    return new discord_js_1.EmbedBuilder()
        .setColor(0x5865f2)
        .setTitle(`Guia — ${info.title}`)
        .setDescription("Use os campos opcionais do `/preview` para testar **antes** de salvar com `/setup`.\n" +
        "Valores não informados usam a config salva do servidor (ou padrão).")
        .addFields({
        name: "Placeholders disponíveis",
        value: (0, suggestions_1.formatPlaceholders)(placeholders).slice(0, 1024),
    }, {
        name: "Cores sugeridas",
        value: (0, suggestions_1.formatColorSuggestions)(),
    }, {
...
function buildFullGuideEmbed() {
    const modules = Object.entries(suggestions_1.MODULE_SUGGESTIONS)
        .map(([key, m]) => `**${m.title}**\n↳ \`${m.previewCommand}\``)
        .join("\n\n");
    const allPlaceholders = [
        ...suggestions_1.PLACEHOLDERS.common,
        ...suggestions_1.PLACEHOLDERS.welcome,
        ...suggestions_1.PLACEHOLDERS.ticket_painel,
        ...suggestions_1.PLACEHOLDERS.ticket_canal,
    ];
    const unique = allPlaceholders.filter((p, i, arr) => arr.findIndex((x) => x.key === p.key) === i);
    return new discord_js_1.EmbedBuilder()
        .setColor(0x5865f2)
        .setTitle("Guia completo de personalização")
        .setDescription("Comandos de preview (só você vê):\n\n" + modules)
        .addFields({
        name: "Todos os placeholders",
        value: (0, suggestions_1.formatPlaceholders)(unique).slice(0, 1024),
    }, {
...
function buildSuggestionsEmbeds(module) {
    if (module === "todos") {
        const main = buildFullGuideEmbed();
        const examples = new discord_js_1.EmbedBuilder()
            .setColor(0x57f287)
