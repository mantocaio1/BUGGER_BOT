...
const defaults_3 = require("../config/defaults");
...
exports.setupCommand = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .addStringOption((opt) => opt
        .setName("mensagem")
        .setDescription(`Texto do embed. ${defaults_1.PLACEHOLDER_HINT}`)
        .setRequired(false))
        ...
        .addStringOption((opt) => opt
        .setName("mensagem")
        .setDescription(`Texto. ${defaults_1.PLACEHOLDER_HINT}`)
        .setRequired(false))
        .addStringOption((opt) => opt.setName("titulo").setDescription("Título do embed").setRequired(false))
        .addStringOption((opt) => opt.setName("cor").setDescription("Cor hex (ex: #ED4245)").setRequired(false))
        .addStringOption((opt) => opt.setName("imagem").setDescription("URL do banner").setRequired(false))
        .addBooleanOption((opt) => opt.setName("usar_embed").setDescription("Usar embed (padrão: sim)").setRequired(false))
        .addBooleanOption((opt) => opt.setName("ativo").setDescription("Ativar/desativar (padrão: ativar)").setRequired(false)))
        .addSubcommand((sub) => sub
        ...
        .setRequired(false))
        .addStringOption((opt) => opt.setName("cor_embed").setDescription("Cor do embed (hex)").setRequired(false))
        .addRoleOption((opt) => opt.setName("cargo1").setDescription("Cargo de cor 1").setRequired(false))
        .addRoleOption((opt) => opt.setName("cargo2").setDescription("Cargo de cor 2").setRequired(false))
        .addRoleOption((opt) => opt.setName("cargo3").setDescription("Cargo de cor 3").setRequired(false))
        .addRoleOption((opt) => opt.setName("cargo4").setDescription("Cargo de cor 4").setRequired(false))
        .addRoleOption((opt) => opt.setName("cargo5").setDescription("Cargo de cor 5").setRequired(false))
        .addRoleOption((opt) => opt.setName("cargo6").setDescription("Cargo de cor 6").setRequired(false)))
        .addSubcommand((sub) => sub
        .setName("roles")
        ...
        .addStringOption((opt) => opt.setName("rotulo1").setDescription("Texto do botão 1").setRequired(true))
        .addStringOption((opt) => opt.setName("emoji1").setDescription("Emoji do botão 1").setRequired(false))
        .addRoleOption((opt) => opt.setName("cargo2").setDescription("Cargo 2").setRequired(false))
        .addStringOption((opt) => opt.setName("rotulo2").setDescription("Texto do botão 2").setRequired(false))
        .addRoleOption((opt) => opt.setName("cargo3").setDescription("Cargo 3").setRequired(false))
        .addStringOption((opt) => opt.setName("rotulo3").setDescription("Texto do botão 3").setRequired(false))
        .addStringOption((opt) => opt.setName("titulo").setDescription("Título do embed").setRequired(false))
        .addStringOption((opt) => opt.setName("mensagem").setDescription("Descrição do painel").setRequired(false))
        .addStringOption((opt) => opt.setName("cor").setDescription("Cor hex").setRequired(false))
        .addStringOption((opt) => opt.setName("imagem").setDescription("URL do banner").setRequired(false)))
        .addSubcommand((sub) => sub
        .setName("ticket")
        ...
        .setDescription(`Texto do painel. ${defaults_1.PLACEHOLDER_HINT}`)
        .setRequired(false))
        .addStringOption((opt) => opt
        ...
        .setDescription("Ativar ou desativar (padrão: ativar)")
        .setRequired(false)))
        .addSubcommand((sub) => sub.setName("ver").setDescription("Mostra todas as configurações atuais."))
        .addSubcommand((sub) => sub
        .setName("reset")
        ...
        .setRequired(true)
        .addChoices({ name: "Boas-vindas (texto)", value: "welcome" }, { name: "Boas-vindas (estilo)", value: "welcome_style" }, { name: "Despedida (texto)", value: "goodbye" }, { name: "Despedida (estilo)", value: "goodbye_style" }, { name: "Ticket painel (texto)", value: "ticket_panel" }, { name: "Ticket painel (estilo)", value: "ticket_panel_style" }, { name: "Ticket canal (texto)", value: "ticket_channel" }, { name: "Ticket canal (estilo)", value: "ticket_channel_style" }, { name: "Logs (cor)", value: "logs_color" }))),
    async execute(interaction) {
        ...
        if (sub === "welcome") {
            ...
            await interaction.reply({
                content: [
                    ...
                    `\n_${defaults_1.PLACEHOLDER_HINT}_`,
                ]
                    .filter(Boolean)
                    .join("\n"),
                ephemeral: true,
        ...
        if (sub === "autorole") {
            ...
            if (testar) {
                ...
                extra = `\n\n**Diagnóstico:**\n${await (0, autoRole_1.diagnoseAutoRole)(member)}`;
            }
            await interaction.reply({
        ...
        if (sub === "goodbye") {
            ...
            await interaction.reply({
                content: [
                    ...
                    message ? "Mensagem personalizada salva." : `Padrão: ${defaults_2.DEFAULT_GOODBYE_MESSAGE}`,
        ...
        if (sub === "ticket-tipos") {
            ...
            const categories = {
                ...(0, store_1.getGuildConfig)(guild.id).ticketTypeCategories,
            };
            if (catSuporte)
                categories.suporte = catSuporte.id;
            if (catDenuncia)
                categories.denuncia = catDenuncia.id;
            if (catParceria)
                categories.parceria = catParceria.id;
            (0, store_1.setGuildConfig)(guild.id, {
        ...
        if (sub === "cores") {
            ...
            const existing = (0, store_1.getGuildConfig)(guild.id).colorPanel;
            const mergedRoleIds = roleIds.length > 0 ? roleIds : (existing?.roleIds ?? []);
            if (mergedRoleIds.length === 0) {
                throw new Error("Informe pelo menos um cargo (cargo1…) ou use `/cores criar` antes.");
            }
            (0, store_1.setGuildConfig)(guild.id, {
                colorPanel: {
            ...
            await interaction.reply({
                content: [
                    ...
                    mensagem ? "Mensagem personalizada salva." : `Padrão: ${defaults_3.DEFAULT_COLOR_PANEL_MESSAGE}`,
        ...
        if (sub === "roles") {
            ...
            const imagem = validateImageUrl(interaction.options.getString("imagem"));
            const buttons = [];
            for (let i = 1; i <= defaults_2.MAX_ROLE_PANEL_BUTTONS; i++) {
                const role = interaction.options.getRole(`cargo${i}`);
                const label = interaction.options.getString(`rotulo${i}`);
        ...
        if (sub === "reset") {
            ...
            (0, store_1.setGuildConfig)(guild.id, patches[modulo] ?? {});
            await interaction.reply({
        ...
        if (sub === "ver") {
            ...
            const fmt = (id) => id
                ...
                : "—";
            const fmtRole = (id) => id ? guild.roles.cache.get(id)?.toString() ?? `\`${id}\`` : "—";
            const onOff = (v) => (v ? "✅ Ativo" : "❌ Inativo");
            await interaction.reply({
                content: [
