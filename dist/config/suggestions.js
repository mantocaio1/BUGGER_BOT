...
exports.MODULE_SUGGESTIONS = exports.COLOR_SUGGESTIONS = exports.PLACEHOLDERS = void 0;
exports.formatPlaceholders = formatPlaceholders;
exports.formatColorSuggestions = formatColorSuggestions;
...
exports.MODULE_SUGGESTIONS = {
    welcome: {
        ...
        setupExample: "/setup welcome canal:#bem-vindos titulo:Bem-vindo! cor:#57F287 imagem:URL mensagem:Olá {user}! Você é o #{count} em {server}.",
    ...
    ticket_painel: {
        ...
        setupExample: "/setup ticket categoria:Tickets suporte:@Staff cor_painel:#5865F2 titulo_painel:Central de atendimento motivo_obrigatorio:true",
    ...
    ticket_canal: {
        ...
        setupExample: "/setup ticket ... titulo_ticket:Seu ticket cor_ticket:#FEE75C mensagem_ticket:Olá {user}!\\n{reason_block}Aguarde {staff}.",
...
function formatPlaceholders(modules) {
    if (modules.length === 0)
        return "Nenhum placeholder específico.";
    return modules
        .map((p) => `\`{${p.key.replace(/[{}]/g, "")}}\` — ${p.description}\n↳ Ex: ${p.example}`)
        .join("\n\n");
}
function formatColorSuggestions() {
    return exports.COLOR_SUGGESTIONS.map((c) => `\`${c.hex}\` **${c.name}** — ${c.use}`).join("\n");
}
