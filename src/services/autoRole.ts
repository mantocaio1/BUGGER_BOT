...
export async function applyAutoRole(member: GuildMember): Promise<AutoRoleResult> {
  ...
  if (!botMember?.permissions.has(PermissionFlagsBits.ManageRoles)) {
    console.warn(
      `[AutoRole] Sem permissão Manage Roles em ${member.guild.name} (${member.guild.id})`
    );
    return {
      applied: false,
      skipped: false,
  ...
  if (!role.editable) {
    console.warn(
      `[AutoRole] Cargo ${role.name} acima do bot em ${member.guild.name}.`
    );
    return {
      applied: false,
      skipped: false,
...
export async function diagnoseAutoRole(member: GuildMember) {
  const config = getGuildConfig(member.guild.id);
  const lines: string[] = [];
  ...
  const botMember = member.guild.members.me;
  lines.push(
    `• Bot tem Manage Roles: ${botMember?.permissions.has(PermissionFlagsBits.ManageRoles) ? "✅" : "❌"}`
  );
  ...
  if (botMember) {
    lines.push(
      `• Posição bot/cargo: bot=${botMember.roles.highest.position} · cargo=${role.position}`
    );
  }
